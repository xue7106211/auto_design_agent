...
> - 字体 不可用 instance → `fixFonts(node, degradationMap)` (font-degradation.md)
>
> 禁止 inline 临时序列。本 reference 中"建立 X frame" / "放入 Y 实例"等指令 = `placeStandardComponent` 调用，不是手写 `createFrame` + `resize`。
