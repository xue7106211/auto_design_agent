...
- Figma 分析帧 `OS4应用框架类型分析` / `node 234:34207` 内 `设置-智能通话笔记禁用` / `设置-智能通话笔记未开启` 各帧（2026-05-14 确认）
- 1depth Phone / Fold外 / Fold内 / Pad 承载形态 与 标题栏 variant 已通过图像直接确认
- 2depth 暂无图像 —— 按 `device-dimensions.md` 规定填写。后续获得截图后再校验。
