...
| 6 | **覆盖模式 遮罩 z-order** | Pad 竖 NLC 覆盖：z-order = `main → 状态栏 → 遮罩-N覆盖 → 分割线 → Sidebar → 杆子`。**遮罩-N覆盖 必须在状态栏之上**（按列归属：N 列以外含状态栏全部 dim）。Sidebar 在遮罩之上（trigger 列豁免）。L 编辑遮罩同理：`遮罩-编辑` 在 `状态栏` 之上。详见 common-rules §3.7 / §3.7a / §3.7b |
| 7 | **栏背景色 token（card-presence rule + N 跟随 LC 规则）** | **L/C 栏**：容器有卡片 / 套卡 list 时 fill = `背景色/surface_low`（灰底）；单一全幅 panel / 不带卡片 flat list = `背景色/surface`（白底）。**N 栏（Sidebar）：跟随 LC 背景色**（与相邻 L 栏一致；L 不存在时跟随 C）—— Sidebar 卡片浮起效果由组件内部结构提供，N 栏自身与 L 视觉连续。**笔记 默认**：L 栏（List_Notes 卡片 stacked）= `surface_low`，C 栏 Detail = `surface`，N 栏 跟随 L = `surface_low`，frame = `surface_low`。**笔记 编辑模式**：L 栏（List_Notes_04 flat 不带卡片）= `surface`，C 栏 = `surface`，N 栏 跟随 L = `surface`，frame = `surface`。**待办**：L 栏 = `surface`（List_Task_03 flat），C 栏 = `surface`，N 栏 跟随 L = `surface`，frame = `surface`。详见 §0.3 + 末尾「栏背景色」表 |
