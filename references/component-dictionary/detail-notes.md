...
本文档是 `figma-component-dictionary` 的组件族 reference，只服务 `componentFamily = DetailNotes`。
...
| --- | --- | --- | --- | --- | --- |
| `Detail_Notes_01` | `561:23029` | `530x626` | 笔记详情正文容器（含封面图、标题、正文） | `2c438f359a098df063155044dbf7470fbca21e36` | `probed` |

## 内部 padding（特殊处理）

`Detail_Notes_01` 的「外层 frame paddingLeft」实测为 **0**，但封面图（`image 675`）距 Detail 左缘恒为 **20dp**。在 `§3.4a` 合算规则中：
