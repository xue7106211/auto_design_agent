...
本文档是 `figma-component-dictionary` 的组件族 reference，只服务 `componentFamily = List`（笔记 / 待办业务列表）。
...
- `List_Task_01` ~ `List_Task_04`（待办子场景，落库后补充本字典）

## 核心结论

- `componentFamily = List`
...
| 变体 | 内置左 / 右 padding |
| --- | --- |
| `List_Notes_01` ~ `List_Notes_06` | 左 12，右 12 |
